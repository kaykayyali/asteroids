...
(() => {
  ...
  function makeShip() { return { x: W() / 2, y: H() / 2, vx: 0, vy: 0, a: -Math.PI / 2, r: 13, cool: 0, inv: 2.6, flame: 0 }; }
  function makeAsteroid(size, x = rand(0, W()), y = rand(0, H())) { const r = [12, 23, 40][size], n = 8 + Math.floor(Math.random() * 5), shape = Array.from({ length: n }, (_, i) => r * rand(.7, 1.15)); const angle = Math.random() * TAU, speed = rand(18, 55) + level * 3; return { x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, a: 0, spin: rand(-1.2, 1.2), size, r, shape }; }
  ...
  function update(dt) {
    ...
    for (const b of bullets) for (const a of asteroids) if (b.life > 0 && a.dead !== true && dist(b, a) < a.r + b.r) { b.life = 0; a.dead = true; award([100, 50, 20][a.size]); burst(a.x, a.y, a.size === 2 ? 20 : 12); sound('boom'); if (a.size) { asteroids.push(makeAsteroid(a.size - 1, a.x, a.y), makeAsteroid(a.size - 1, a.x, a.y)); } }
